...
class SQLitePipeline(object):
	...
	def __init__(self):
		...
		self.cursor.execute('CREATE TABLE IF NOT EXISTS collegeimages(name TEXT, state TEXT, city TEXT)')

	# take the Item and put it in database
	def process_item(self, item, spider):
		self.cursor.execute("INSERT OR REPLACE INTO collegeimages(name, state, city) values (?, ?, ?)", (item['name'].strip(), item['state'].strip(), item['city'].strip()))
